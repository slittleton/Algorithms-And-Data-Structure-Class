...
class SinglyLinkedList {
   ...
   * a->b->c->d->e->f->null
   * process of reversal with loop
   * variables keep track of gap: prev, node, next
   *   p    n nx
   * null <-a b-> c-> d-> e-> f
   *        p   n nx
   * null <-a <-b c-> d-> e-> f
   ...
   * null <-a <-b <-c <-d <-e f
   *
   * null <-a<-b<-c<-d<-e<-f -----fully reversed
   *
   * output: f->e->d->c->b->a->null
   */
  reverse() {
    let node = this.head;
    this.head = this.tail;
    ...
    for (var i = 0; i < this.length; i++) {
      // the first loop, prev and node are already set
  ...
  // singly linked list reversal with while loop
  reverseW() {
    let node = this.head;
    let previous, temp;
    while (node) {
  ...
  // recursive singly linked list reversal
  recurRev() {
